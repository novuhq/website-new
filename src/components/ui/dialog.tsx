...
const hasDialogCloseRecursive = (children: React.ReactNode): boolean => {
  return React.Children.toArray(children).some((child) => {
    ...
    if ((child.props as React.PropsWithChildren).children) {
      return hasDialogCloseRecursive(
...
function DialogContent({
  className,
  children,
  "has-custom-close": customClose = false,
  ...props
}: React.ComponentProps<typeof DialogPrimitive.Content> & {
  "has-custom-close"?: boolean
}) {
  const hasCustomClose = hasDialogCloseRecursive(children) || customClose
