...
function InboxContainer({
  ...
  const [activeTab, setActiveTab] = useState(categories ? categories[0] : "")
  ...
  return (
    <m.div
      className={cn(
        "absolute left-0",
        variant === "comparison"
          ? "top-0 hidden lg:block"
          : "top-14 hidden md:block"
      )}
    ...
    >
      <div
        className={cn(
          "relative z-30 w-[531px] overflow-hidden rounded-[20px] xl:w-[608px]",
          variant === "comparison" && "px-5"
        )}
      >
        {variant === "comparison" && (
          <header className="relative z-40 flex h-[46px] items-center">
            ...
            {NotificationIcon && (
              <div className="relative mr-3.5 shrink-0">
                ...
                />
                {isUnreadMessages && (
                  <div className="absolute top-1 right-[3px] size-2 rounded-full border border-[#151b37] bg-[linear-gradient(232.66deg,#FFDF66_9.72%,#FFB433_89.91%)]" />
                )}
              </div>
            )}
            {ThemeIcon && (
              <ThemeIcon
                className={cn(
                  "mr-3.5 size-6 shrink-0",
                  currentTheme.header.icon
                )}
              />
            )}
            {MessageIcon && (
      ...
      {variant === "comparison" && (
        <div
          className={cn(
            ...
            "absolute top-0 left-0 z-10 aspect-[608/573] h-[501px] w-auto shrink-0 rounded-[20px] xl:h-[573px]",
            theme !== "linearDark" && theme !== "linearLight" && "p-px"
          )}
        >
          <div
          ...
          {currentTheme.mainBlock.shine && (
            <>
              <div
                className={cn(
                  currentTheme.mainBlock.shine,
                  "pointer-events-none absolute top-0 left-0 z-30 h-px w-[380px] mix-blend-plus-lighter blur-[5px]"
                )}
                aria-hidden
              />
              <div
                className={cn(
                  currentTheme.mainBlock.shine,
                  "pointer-events-none absolute top-0 left-0 z-30 h-px w-[380px] mix-blend-plus-lighter blur-[3px]"
                )}
                aria-hidden
              />
              <div
                className={cn(
                  currentTheme.mainBlock.shine,
                  "pointer-events-none absolute top-0 left-0 z-30 h-px w-[380px] mix-blend-plus-lighter blur-[1px]"
                )}
                aria-hidden
              />
            </>
