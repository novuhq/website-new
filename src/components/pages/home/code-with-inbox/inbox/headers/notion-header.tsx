"use client"

import { cn } from "@/lib/utils"
import { NotionMoreIcon, NotionSettingsIcon } from "../icons"
import { NOTION_HEADER_THEMES } from "../theme-config"
import type { IFilterItem, IActionItem } from "../types"

interface INotionHeaderProps {
  theme: "notionDark" | "notionLight"
  filters: IFilterItem[]
  actions: IActionItem[]
  handleAction: (action: IActionItem["action"]) => () => void
  handleFilter: (index: number) => () => void
}

function NotionHeader({
  theme,
  filters,
  actions,
  handleAction,
  handleFilter,
}: INotionHeaderProps) {
  const currentTheme = NOTION_HEADER_THEMES[theme]

  return (
    <div className="relative z-20 mb-1 mt-5 flex shrink-0 items-start justify-between px-[26px]">
      <span
        className={cn(
          currentTheme.titleColor,
          "inline-block font-inter text-xl font-semibold leading-none"
        )}
      >
        Inbox
      </span>
      <div className="flex items-center gap-x-6">
        <button
          className="group relative flex h-[30px] items-center pb-2.5"
          type="button"
        >
          <NotionMoreIcon
            className={cn("h-1.5 w-[18px]", currentTheme.mainIconsColor)}
            aria-hidden
          />
          <ul
            className={cn(
              "invisible absolute right-0 top-full w-[219px] gap-y-0.5 rounded-[10px] px-1 py-0.5 opacity-0 transition-[opacity,visibility] duration-300 group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100",
              currentTheme.menuStyles
            )}
          >
            {actions.map(({ label, Icon, action }, index) => (
              <li key={index}>
                <div
                  className={cn(
                    "flex h-[30px] w-full items-center gap-x-2.5 rounded-md pl-4 text-left",
                    currentTheme.menuItemStyles
                  )}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) =>
                    e.key === "Enter" && handleAction(action)()
                  }
                  onClick={handleAction(action)}
                >
                  <Icon className="size-[18px]" aria-hidden />
                  {label}
                </div>
              </li>
            ))}
          </ul>
        </button>
        <button
          className="group relative flex h-[30px] items-center pb-2.5"
          type="button"
        >
          <NotionSettingsIcon
            className={cn("size-4", currentTheme.mainIconsColor)}
            aria-hidden
          />
          <ul
            className={cn(
              "invisible absolute right-0 top-full w-[219px] gap-y-0.5 rounded-[10px] px-1 py-0.5 opacity-0 transition-[opacity,visibility] duration-300 group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100",
              currentTheme.menuStyles
            )}
          >
            {filters.map(({ label, Icon }, index) => (
              <li key={index}>
                <div
                  className={cn(
                    "flex h-[30px] w-full items-center gap-x-2.5 rounded-md pl-4 text-left",
                    currentTheme.menuItemStyles
                  )}
                  tabIndex={0}
                  role="button"
                  onKeyDown={(e) =>
                    e.key === "Enter" && handleFilter(index)()
                  }
                  onClick={handleFilter(index)}
                >
                  <Icon className="size-[18px]" aria-hidden />
                  {label}
                </div>
              </li>
            ))}
          </ul>
        </button>
      </div>
    </div>
  )
}

export default NotionHeader
