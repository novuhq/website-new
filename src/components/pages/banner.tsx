...
function Banner({ title, description, className, cta }: IBanner) {
  return (
    <section className={cn("banner relative", className)}>
      <div className="relative mx-auto flex max-w-240 items-center justify-between gap-x-8 rounded-xl p-10">
        ...
        <Button className="relative z-20 shrink-0" size="default" asChild>
          <NextLink href={cta.href}>{cta.label}</NextLink>
        </Button>

        <div className="absolute inset-0 z-10 overflow-hidden rounded-[inherit]">
          <Image
            className="pointer-events-none absolute inset-0 min-h-full max-w-none min-w-full object-cover"
            src={bgIllustration}
            width={960}
            height={186}
            quality={100}
