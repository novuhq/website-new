...
function Cta({ text, buttonText, buttonUrl }: IContentCtaBlock) {
  return (
    <div className="not-prose relative my-6 flex items-center justify-between gap-x-4 overflow-hidden rounded-xl px-4 py-6 pb-5 md:px-5">
      <h2 className="text-[20px] leading-tight font-medium md:text-[22px]">
        {text}
      </h2>
      ...
      <span
        className="absolute inset-0 -z-1 rounded-xl border-gradient bg-[radial-gradient(circle_at_top_right,rgba(236,209,250,0.3)_11%,rgba(95,82,122,0.3)_50%,rgba(168,148,209,0.1)_100%)]"
        aria-hidden
      />
      <Image
        className="absolute inset-0 -z-10 min-h-full w-auto"
        src={bgIllustration}
        width={704}
        height={84}
        quality={100}
      ...
      />
      <Image
        className="absolute -top-[9px] -right-[11px] -z-1"
        src={shine}
        width={276}
        height={87}
        alt=""
