...
test("sends to the original agent, streams a reply, and preserves a failed draft for retry", async ({
  ...
  expect(novu.sessions[0]).toMatchObject({
    subscriber: {
      subscriberId: expect.stringMatching(
...
test("isolates signed subscribers between visitors and reuses each visitor cookie", async ({
  page,
  browser,
}) => {
...
for (const sessionFailure of ["unavailable", "unsigned"] as const) {
